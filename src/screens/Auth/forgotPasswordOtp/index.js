  ...
  const otpVarify_handle = async () => {
    if (code) {
      ...
      try {
        ...
        if (result.status == 200) {
          const data = result.data;
          console.log({forgotOtp:data});
          if (data.ResponseId == -1) {
            setPopupMessageVisibility(true);
            setTitle('Sorry');
            setSubtitle('This otp is not valid');
          } else if (data.ResponseId == -2) {
            setPopupMessageVisibility(true);
            setTitle('Sorry');
            setSubtitle('This otp has been expired, Please try to resend otp');
          } else {
            navigation.navigate('ChangePassword',{UserNewCode:data.UserNewCode});
            setOtp('');
          }
          setLoading(false);
        }
